...
- 2026-09-19 · orchestrator · Reviewer agent has no Write tool, so its `memory: project` never persists → reviewer gotchas are kept by the orchestrator in `docs/review-notes.md`; reviewer.md now reads it at start and ends reports with a NOTES line.
- 2026-09-19 · orchestrator · Removing `infra-ready` (M0-T4 brief 4a) made base-file `make up` exit non-zero → Makefile `up`/`down` now drive the two-file stack (infra + four services); one-file scope extension granted to the M0-T4 coder rather than a new brief.
- 2026-09-19 · orchestrator · M1 dependency graph (T2,T5,T6←T1; T3←T2; T7←T3,T4) → M1 runs as four waves: 1 = T1+T4, 2 = T2+T5+T6, 3 = T3, 4 = T7 (PLAN estimated two; the estimate was wrong, the deps are not).
- 2026-09-19 · orchestrator · M1-T4 (ledger) lives in `services/case` (HLD §4: case service owns the audit ledger) and takes postgres/flyway/testcontainers versions from the Spring Boot BOM so it does not touch the version catalog in the same wave as M1-T1.
- 2026-09-19 · orchestrator · M1-T1 picks up both deferred M0-T1 MAJORs (leaf-only Gradle conventions; JUnit/AssertJ versions into the catalog).
- 2026-09-19 · orchestrator · HLD §9 says "compacted topics per case"; compaction keeps only the latest record per key and would defeat replay → M1-T2 uses one topic per event type (topic name = wire name), keyed by caseId, `cleanup.policy=delete`, 90-day retention. Flagged to the human in QUESTIONS.md.
- 2026-09-19 · orchestrator · "Tenant id on every record" (HLD §9) is enforced at the persistence boundary (every stored row/document carries tenant_id and implements HasTenant), not by adding tenantId to every domain record; Case remains the tenant root. M1-T6 documents the rule.
- 2026-09-19 · orchestrator · Reviewer flagged statute tokens ("s.71A", "SAR proceeding") in `EncumbranceType` wire names → renamed to `RESTORATION_CLAIM` / `REVENUE_COURT_PROCEEDING`; rule: statute names live only in packs, institution names (SAR/DCLR/CO forums) and tenure vocabulary (kism values) may stay in the model.
